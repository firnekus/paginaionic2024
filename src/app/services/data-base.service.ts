...
export class DataBaseService {
  ...
  async guardarUsuario(usuario: Usuario): Promise<void> {
    ...
    console.log(this.sqlInsertUpdate, [usuario.cuenta, usuario.correo, usuario.password,
      usuario.preguntaSecreta, usuario.respuestaSecreta, usuario.nombre, usuario.apellido,
      NivelEducacional.buscarNivelEducacional(usuario.nivelEducacional.id), new Date(usuario.fechaNacimiento!).toISOString(  ), usuario.direccion])
    console.log(usuario.nivelEducacional.id)
    await this.db.run(this.sqlInsertUpdate, [usuario.cuenta, usuario.correo, usuario.password,
      usuario.preguntaSecreta, usuario.respuestaSecreta, usuario.nombre, usuario.apellido,
      usuario.nivelEducacional.id, new Date(usuario.fechaNacimiento!).toISOString(  ), usuario.direccion]);
    await this.leerUsuarios();
  ...
  async cagasteConCorreo(correo: string) {
  ...
  async eliminarUsuarioUsandoCorreo(correo: string): Promise<void> {
    await this.db.run('DELETE FROM USUARIO WHERE correo=?', 
      [correo]);
    await this.leerUsuarios();
  }
